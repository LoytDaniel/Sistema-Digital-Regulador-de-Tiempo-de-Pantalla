...
module LCD12864_controller_prueba1 #(
...
screen_builder_pixel_prueba inst(
    .current_hr(current_hr), 
	 .current_min(current_min), 
    .current_hr_pixel_dec(current_hr_pix_dec), 
	 .current_hr_pixel_uni(current_hr_pix_uni),
    .current_min_pixel_dec(current_min_pix_dec), 
	 .current_min_pixel_uni(current_min_pix_uni)
);
...
always @(*) begin
    for (i = 0; i < NUM_DATA_ALL; i = i + 1) begin //Carga el arreglo de memoria gráfica con la pantalla correspondiente
        if (adult) graphic_mem[i] = adult_mem[i];
    end
